  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`relative w-full rounded-none border p-6 transition-all duration-200 overflow-hidden ${
          isExpired
      ...
      >
        ...
        <div className="flex items-start justify-between relative z-10">
          <div>
            ...
            <h3 className="font-header font-bold text-xl text-alabaster mt-2 tracking-tight">
              {pass.name}
            </h3>
          </div>
        ...
        <div className="mt-5 p-3.5 bg-dark border border-dark-border relative z-10">
          <div className="flex items-center justify-between text-xs font-mono mb-2">
            <span className="text-zincGrey flex items-center space-x-1.5">
              <Clock className="w-3.5 h-3.5 text-uranium" />
              <span>TIME REMAINING:</span>
            </span>
            <span className={`font-bold ${isUrgent ? "text-aviation" : isFresh ? "text-uranium" : "text-alabaster"}`}>
              {remainingDays} DAYS / {totalDays}D ({percentLeft}%)
            </span>
          </div>

          {/* Custom Decay Progress Bar */}
          <div className="w-full h-2 bg-dark-surface border border-dark-border overflow-hidden relative">
            <div
        ...
        {/* Technical Data HUD */}
        <div className="mt-4 grid grid-cols-2 gap-2 text-xs font-mono relative z-10">
          <div className="p-2 border border-dark-border bg-dark">
            <span className="text-[10px] text-zincGrey block uppercase">Original Retail</span>
            <span className="text-zincGrey-light line-through">{pass.originalPriceEth} ETH</span>
          </div>
          <div className="p-2 border border-dark-border bg-dark-surface">
            <span className="text-[10px] text-uranium block uppercase">Current Value</span>
            <span className="text-uranium font-bold text-sm">{effectivePrice} ETH</span>
          </div>
        </div>

        {/* Barcode & Stylus Verification Line */}
        <div className="mt-5 pt-3 border-t border-dashed border-dark-border flex items-center justify-between relative z-10">
          <div className="flex items-center space-x-1 text-[10px] font-mono text-zincGrey">
            <ShieldCheck className="w-3.5 h-3.5 text-periwinkle" />
        ...
        {showActions && (
          <div className="mt-5 flex items-center space-x-2 relative z-10">
            <Link
              ...
              className="flex-1 text-center py-2 px-3 border border-dark-border bg-dark hover:bg-dark-surface text-alabaster font-mono text-xs uppercase tracking-wider transition-colors"
            >
              Inspect Detail
            </Link>
            {pass.isListed && onBuy && (
              <button
                onClick={() => onBuy(pass)}
                className="flex-1 flex items-center justify-center space-x-1 py-2 px-3 bg-uranium hover:bg-uranium-glow text-black font-mono text-xs font-extrabold uppercase tracking-wider transition-all hover:shadow-glow-uranium"
              >
